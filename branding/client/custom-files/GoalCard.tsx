...
export default function GoalCard({ goal, siteId }: GoalCardProps) {
  ...
  const handleFilterByGoal = () => {
    // Ensure we have valid values before creating filter
    if (goal.goalType === "event" && !goal.config.eventName) return;
    if (goal.goalType === "path" && !goal.config.pathPattern) return;

    const filter = goal.goalType === "event"
      ? {
        parameter: "event_name" as const,
        type: "equals" as const,
        value: [goal.config.eventName!],
      }
      : {
        parameter: "pathname" as const,
        type: "equals" as const,
        value: [goal.config.pathPattern!],
      };
        ...
        >
          ...
          <div className="flex shrink-0 gap-1 pl-4">
            <div onClick={e => e.stopPropagation()}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="smIcon" onClick={handleFilterByGoal}>
