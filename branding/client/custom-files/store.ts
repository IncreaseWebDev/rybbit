...
export const addFilter = (filter: Filter) => {
  ...
  if (filterIndex === -1) {
    // No matching filter exists, add new one
    setFilters([...filters, filter]);
  } else {
    // Matching filter exists, merge values (deduplicate using Set)
    const existingFilter = filters[filterIndex];
    const existingValues = Array.isArray(existingFilter.value)
      ? existingFilter.value
      : [existingFilter.value];
    const newValues = Array.isArray(filter.value)
      ? filter.value
      : [filter.value];
